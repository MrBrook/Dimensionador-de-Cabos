...
import static br.com.gui.PrincipalTela.carregaArr;
...
public class SubTela extends JFrame {
	...
	public Circuito circuitos = new Circuito();
	public ArrayList<Circuito> c = new  ArrayList<Circuito>();


	public SubTela(Controle controle) throws InterruptedException {

		//PrincipalTela principalTela = new PrincipalTela(n);
		...
		lblNewLabel_2.setBounds(8, 278, 325, 14);
		panel.add(lblNewLabel_2);

		JLabel lblPower = new JLabel("Power");
		lblPower.setFont(new Font("Tahoma", Font.PLAIN, 13));
		lblPower.setBounds(141, 298, 54, 22);
		panel.add(lblPower);

		txtWatts = new JTextField();
		txtWatts.setText("4000");
		txtWatts.setBounds(66, 328, 105, 20);
		panel.add(txtWatts);
		...
		cConductorSch.setBounds(126, 360, 207, 20);
		...
		label_4.setBounds(10, 328, 54, 22);
		panel.add(label_4);

		Label label_5 = new Label("Conductor scheme:");
		label_5.setBounds(10, 358, 117, 22);
		panel.add(label_5);
		...
		bFinish.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(txtWatts.getText()!=null){
					try{
						float potencia = Float.parseFloat(txtWatts.getText());
						int tensao = Integer.parseInt(cVoltage.getSelectedItem().replace(" V",""));
						float fp = Float.parseFloat(txtPF.getText());

						circuitos.setNome(tNomeCircuito.getText());
						circuitos.setTensao(tensao);
						circuitos.setMaterial(cMaterial.getSelectedIndex()+1);
						circuitos.setMetodo(cMInstallation.getSelectedIndex()+1);
						circuitos.setPotetencia(potencia);
						circuitos.setFatorPotencia(fp);
						circuitos.setTipo(group.getSelection().getActionCommand());
						circuitos.setMetodo(cMInstallation.getSelectedIndex()+1);

						if(cConductorSch.getSelectedIndex()<3){
							circuitos.setnConectores(2);
							circuitos.setIb((float) (potencia/(tensao*fp)));
						}
						else if(cConductorSch.getSelectedIndex() == 3){
							circuitos.setnConectores(2);
							circuitos.setIb((float) (potencia/(tensao*fp)));
						}
						else{
							circuitos.setnConectores(3);
							circuitos.setIb((float) (potencia/(tensao*fp*Math.sqrt(3))));
						}
						circuitos.setIsolacao(cIsolation.getSelectedIndex()+1);

						try{
							c.add(circuitos);
							carregaArr(c);
							controle.setCircuito(true);
						}catch (Exception s){
						...
						JOptionPane.showMessageDialog(panel,"Campo invalido!!");
					}
				}else {
					JOptionPane.showMessageDialog(panel,"Campo invalido!!");
				}
